...
func _init() -> void:
	...
	check(all_ok, "mode_label covers all 23 modes")
	...
	var want_ids = ["daily", "time_attack", "memory", "frost", "zen", "hell", "moves", "race", "stack", "gravity", "fog", "chain", "fever", "perfect", "tray", "collect", "flip", "rock", "defuse", "target", "shift", "endless"]
	var state := {
	...
	for r in rows:
		got_ids.append(r["id"])
	check(got_ids == want_ids, "modes_panel_rows returns 22 rows in panel order")
	var titles_ok := true
	for r in rows:
