...
func _init() -> void:
	...
	check(all_ok, "mode_label covers all 25 modes")
	...
	check(SM.RECORD_MODES.size() == 21, "record table covers 21 modes")
	var table_ok = true
	var ach_ok = true
	...
	var want_ids = ["daily", "time_attack", "memory", "frost", "zen", "hell", "moves", "race", "stack", "gravity", "fog", "chain", "fever", "perfect", "tray", "collect", "flip", "rock", "defuse", "target", "shift", "slide", "defense", "endless"]
	var state := {
	...
	for r in rows:
		got_ids.append(r["id"])
	check(got_ids == want_ids, "modes_panel_rows returns 24 rows in panel order")
	var titles_ok := true
	for r in rows:
	...
	check(DATA.DEFAULT_CONFIGS.size() == 24, "data module carries 24 mode configs")
	var inv_ok = true
	var unlock_too_high = ""
