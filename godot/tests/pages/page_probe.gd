...
func _init() -> void:
	...
	check(game.tree_buff_offer_open, "endless opens the buff interlude after the clear")
	check(game.level_advance_timer.is_stopped(), "the next round waits for the buff pick")
	game._on_tree_buff_skipped()
	check(game.level_advance_timer && !game.level_advance_timer.is_stopped(), "the pick resolve schedules the next round")
	check(not game.tree_buff_offer_open, "the skip closes the interlude")
