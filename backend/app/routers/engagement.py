...
def list_achievements(user_id: str = Depends(current_user_id)):
    return {"achievements": engagement_service.list_achievements(user_id)}


@router.post("/achievements/{achievement_id}")
def unlock(achievement_id: str, user_id: str = Depends(current_user_id)):
    return {"newly_unlocked": engagement_service.unlock(user_id, achievement_id)}


@router.get("/missions")
def list_missions(week_key: int = Query(..., ge=0), user_id: str = Depends(current_user_id)):
    return {"missions": engagement_service.list_missions(user_id, week_key)}


@router.put("/missions")
def upsert_mission(payload: MissionPutRequest, user_id: str = Depends(current_user_id)):
...
@router.get("/wallet")
def wallet(user_id: str = Depends(current_user_id),
...
def wallet_append(payload: WalletEntryRequest, user_id: str = Depends(current_user_id)):
    return engagement_service.wallet_append(user_id, payload.delta, payload.reason)


@router.post("/signin")
def signin(payload: SigninRequest, user_id: str = Depends(current_user_id)):
...
@router.get("/signin")
def list_signins(user_id: str = Depends(current_user_id),
