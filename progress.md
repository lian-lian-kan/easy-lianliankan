...
## 2026-09-12 (重构 Round CK：棘轮清偿——19 个存量超长函数全部拆至 45 行内)
- 动机：用户定向「质量可交付、可继续新版本迭代」。CJ 轮棘轮表登记的 19 个存量超标函数（最大 update_layout 154 行）逐个清偿，全仓函数长度红线（45 行 ERROR / 35 警告）自此无存量豁免。
- 手法：同文件语义块原样搬移（锚点断言 + 缩进提升），全程 shell_audit 秒级自查；两处结构性改写——①game_input._unhandled_input 的 19 分支 match 改 KEY_ACTIONS 键位表驱动（method/playing 门控/arg 统一分发，Escape 因仅全屏时消费保留专属分支）；②powerups._use_power_up 的 match 改 `_activate_<type>` 命名约定 call 分发、三处收回守卫收敛 _recall_armed 表驱动（rainbow 附带清选中保留特例）。另有两处顺带去重：_on_tile_pressed/_on_memory_tile_pressed 的双 mismatch 分支各收敛为单一 helper；hud_timers._build_timers 11 个 Timer 改 TIMER_SPECS 规格表（GDScript 字典保序，add_child 次序不变；注意 connect 目标进表后 shell_audit 检查 2 不再静态覆盖这批回调，由 hud_timers 行为探针兜底）。
- 清偿明细（旧→新行数）：update_layout 154→16、apply_update 117→12、_reset_level_session 110→19、_on_tile_pressed 82→24、normalize_progress 79→25、_unhandled_input 62→18、build_view 67→11、_refresh_board_visuals 70→14、find_path 60→30、_on_memory_tile_pressed 59→16、_pause_panel 59→31、_settings_panel 63→36、_onboarding_panel 51→35、_use_power_up 52→16、_settle_campaign_clear 51→14、_build_controls_flow 51→17、_build_stats 50→11、_build_board_area 49→17、_build_timers 47→9。
...
## 2026-09-19 (棋盘铺满屏幕：同牌数形状适配 + 有界弹性瓦片)

- **背景**：用户质疑桌面宽屏下棋盘两侧留白——"为什么不按屏幕决定行列，而是留白？"复盘确认：正方形瓦片不是规则是实现选择；棋盘长宽比与窗口长宽比的错配被原样留成了留白。
- **方案（玩法零改动）**：牌数、图案种数、时限是难度契约（排行榜/星级/进度都依赖），**行列排布是自由的**——
  ① 新模块 scripts/board/board_fit.gd：pick() 遍历同牌数的全部因子形状（行列 ≥4），取与棋盘画布长宽比 log 距离最小者（等价于弹性钳制后瓦片形变最小的摆法）；level_with_fitted_shape() 只换 rows/cols、拷贝字典不动原表；game._create_playable_board 在树内取视口尺寸接入（树外/无头测试、工坊 custom_grid、lock_shape 全部直通）。
  ② 画布区长宽比按视口/0.9 计（对应棋盘画布 ≥90% 视口高的既有契约），避免手机选出 4 列竖条蛇形。
  ③ board_view 瓦片改有界弹性矩形：宽高各自 clamp 到方形的 1.6 倍内，吃掉形状适配后的残余错配；字体仍按方形基准算，字形不裁切。
  ④ 连线消 lock_shape（一笔拖链密度是 8x8 设计的一部分）；效果定位读按钮真实矩形，非均匀瓦片天然兼容。
...
- Validation: CI deploy 全绿（54 项无头测试含 board_fit_test + shell_audit + 导出 + 冒烟 + Pages 部署，12d843e）；线上真 Chrome 1280x720 实测：第 1 关以 8 列 x 6 行铺满画布、两侧留白消失，消除/计分/剩余对数回归正常。弹性瓦片首版曾在 CI 抓出 4px 末行越界（panels_probe），已修。
...
## 2026-09-19 (续二：彻底铺满——网格由屏幕推导，正方形约束移除)

- **用户定调**：两侧不要留白；列数行数直接按屏幕决定；瓦片不必正方形。前一轮"同牌数因子形状 + 1.6x 弹性"仍保留了残边，本轮按用户模型重做：
- **board_fit.gd 重写**：grid_for(牌数, 宽, 高) 直接从棋盘画布尺寸搜整数网格——行列可行域由瓦片可读尺寸带（34~220px）夹出，约束"偶数张 ≥ 种子牌数"，评分"额外对数 ×10 + 形变 ×1（超 1.6x 形变 ×40 惩罚）"。棋盘画布仍按视口 ×0.9 估（HUD 契约）。效果：16:9 桌面 8 列×6 行恰好铺满零额外；竖屏手机 6 列×8 行；4K 大屏自动多发牌（48 → 162 张，~213px 瓦片）而不是放大留白。
- **时限按牌数等比缩放**（time_limit>0 时 ×N'/N），大屏多发牌不吃亏；发牌本身天然安全——create_board 本就是成对发牌 + 奇数尾格置空，任意行列都可解（新增测试钉死：9×6/种类 6 不可整除时每种仍成对）。
...
- **续（多分辨率实测：不同的人看到不同的行列）**：真 Chrome 三种视口开局实测（bc91341 构建）——1280x720 / 1920x1080 (16:9) → 8 列 x 6 行（瓦片随屏幕物理放大）；390x844 手机竖屏 → 6 列 x 8 行；2560x1080 (21:9) → 12 列 x 4 行。行列随屏幕长宽比自动推导，全部零侧边留白、零额外牌（16:9/竖屏恰好 48 张）。消除/计分/剩余对数回归正常。同长宽比不同物理尺寸的屏幕（1280 vs 1920）行列相同、瓦片物理大小随屏幕缩放——同一形状布局一致是刻意设计（stretch 逻辑视口恒定）。
- **SW 更新体验修复**：shell 补 controllerchange -> 单次 reload（sw.js 本就 skipWaiting+claim）——此前每次部署后当前访问继续玩旧缓存、要访问两次才能见新版，用户因此误以为适配没做。此后部署即见。
- Validation: CI deploy 全绿（be5c0a9 shell 修复）；三视口真 Chrome 实测行列各不相同、铺满零留白；消除回归绿。

## 2026-09-19 (续三：更新可见性——玩家浏览器停留在旧包的问题)
...
- **续（Round 28 后段）：startup_probe「尾帧噪音」定根因——真产品 bug：金光目标重挑在按钮网格重建前刷新**。Round 27 遗留的 `Invalid get index '6'` 此前被诊断为 quit 尾帧残留回调，本轮埋点（网格边界守卫 + 失配打印）实证两件事：① 错误并非发生在 quit 之后——**stderr 无缓冲、stdout 块缓冲，2>&1 重定向到文件时 stdout 尾块后置**，把 target 进入时刻的 SCRIPT ERROR 挤到了 ALL PASSED 之后，制造了"尾帧"假象；② 真实触发点是 `_pick_target_pair`（session.gd）——开局路径在 `_reset_board_session` 里换装新盘（board_fit 适配后行列常与上一局不同，探针里 10x8 vs 旧 8x6）后立即重挑金光目标，而该函数尾部有一发 `_refresh_board_visuals`：此刻按钮网格还是上一局的形状，新盘 10 行索引旧 8 行网格即越界、刷新中断（生产表现为进指定连消棋盘渲染残缺）。对局中重挑（pair_select._refresh_target_pair）路径末尾本就有覆盖性刷新，此发刷新对 mid-play 冗余、对 reset 有害——**修法：`_pick_target_pair` 改纯状态变更（删除尾部刷新），两处调用方的既有刷新点各自覆盖**。board_view 未加形状守卫：单写入方 `_render_board` 保证网格一致，静默吞越界只会把未来的失配 bug 变成隐身。
- **顺带清偿**：startup_probe 里前会话遗留的 58 行 DBG 调试打印（每模式 pre/done 各一条 + 入场失败诊断块）全部移除。
- Validation: startup_probe RC=0、180 断言全 ok、ALL PASSED、**零 SCRIPT ERROR 零 DBG**（此前 1 条噪音）；全量 58 项无头测试重跑 + port_shell_audit clean 后回填。全量实跑实录：56 项在跑批内绿，page_router_test / offscreen_font_probe 两项 rc=255 但输出仅 "Error initializing GLAD"（当天累计 ~150 次 Godot 启动后本机 GL 上下文创建抖动，跑批脚本带 GLAD 专用重试仍耗尽），GL 恢复后两文件单独复跑立即全绿、零断言/解析错误——判定为环境抖动非代码回归。
- **CI 回填（42b57d9）**：Deploy to GitHub Pages 全绿（58 项无头测试含分域路径 + python 版 shell_audit 八项 + 导出 + 冒烟 + Pages 部署）+ Build Android APK 绿。Round 18-28 批次至此本地、远端双确认。

## 2026-09-23 (Round 29：玩法扩展三连——规则族/肉鸽无尽/Boss 战 + 联网对战立项 + 解锁曲线真 bug)

- Context: 用户定向"玩法太少，要扩展更多更强的玩法"。盘点后选四件事：①规则级合十家族热身（每个半天级）②肉鸽无尽（tree_buffs 基建白送）③Boss 战（defense 骨架）④联网对战立项评估。产品审查清单（.agents/skills/product-review）全程在场：文案只复用既有字符（本机无 python 重跑不了 subset_fonts），层级/闭环走既有形态（玩法卡自动派生、增益复用 L3 弹窗）。
- **新工具 tools/check_copy_chars.pl**：与 subset_fonts.py 取材对齐的本地字符覆盖对拍（无 python 开发机用）。踩两坑记录在案：Windows argv 传中文被代码页搅碎（改走 UTF-8 文件输入）；File::Find 默认 chdir 使相对路径 open 失效、池子小得像缺字泛滥（no_chdir 修）。最终所有新文案对照 git HEAD 旧字符池验证全覆盖（1395 字符池）。
- **①规则族 diff1 差一消（🎱）+ mult 倍数消（🎲）**：谓词进 board_pathfinder.values_match（|a-b|=1 / 成倍），牌面进 board_engine.apply_diff1_faces（类型拆 (t,t+1)）/apply_mult_faces（2-4/2-6/2-8/3-6/3-9/4-8 除子集，1/5/7 不发），game._is_sum_mode 语义升级为 _is_rule_mode 三模式共用一条接线（渲染数字/牌面分发/拒绝提示分模式文案）。解锁 11/12 级。
- **②肉鸽无尽**：_advance_endless_round 过关后改为进「轮间休整」三选一（复用 tree_buffs.roll_offer/offer_tree_buffs 面板/_resolve_tree_buff_pick 全链），tree_buffs.gd roll_offer 加排除参数——无尽无时钟，time_gift/tool_breeze 不进奖池；面板标题/跳过文案随会话刷新（树=层间休整/无尽=轮间休整，成员 tree_buff_title/tree_buff_skip）。挑选或跳过（""）都由 resolve 踢推进计时器——page_probe 的无尽断言随之改写为"offer 先行、skip 后踢表"。
- **③boss Boss挑战（👾，15 级解锁）**：classic-style 一行配置（boss_hp=24 + 复用 defense_start/step）+ CLASSIC_OPTIONAL_FIELDS 透传 boss_hp。每消一对打一下（pair_select._boss_match_hook：扣血→打空调 game._resolve_boss_defeated 即时结算，盘面无需清空）；血线 2/3、1/3 各狂暴一次（逼近提速 defense_step-3 + swap_random_faces×2N 盘面干扰，swap 保 parity 可解性不变）。hud_timers 逼近时钟抽成 _tick_menace 供 defense/boss 共用（顺带把 _on_second_tick 缩回棘轮线内）。结算走 record_modes 通用通路（boss_result/boss_best_score/boss_first 自动派生）。interactions 禁反向 preload——击败经 game 薄壳进会话域。
- **④联网对战立项**：docs/online-battle-design.md——MVP=HTTP 轮询同盘竞速 1v1（同 seed 各自生成、服务器只同步进度、Redis 房间 + PG 冷账本、race 骨架复用），WebSocket 升级留二期，WebRTC 否决；M1 后端 ~3 天/M2 客户端 ~4 天/M3 联调灰度 ~2 天。
